def itus_iono_optimize(data, tx, rx, f0, B, E=None):
    ...
    dechirpped = itus_iono_rng_comp(data, chirp, meanE, f0, B)  
    
    return dechirpped, meanE, histE
...
def itus_iono_rng_comp(data, chirp, E, f0, B, baseband=False):
    ...
    # separate the italian chirp function into its magnitude and phase components in the time domain
    if baseband is True:
    ...
    dechirpped = np.fft.ifft(np.multiply(temp3, np.conj(corr_chirp)))
    dechirpped = np.pad(dechirpped, ((0,0),(0,1552)), 'constant', constant_values=0)
...
def ita_comp(tx,rx,uncompressed):
    ...
    if txval<0: prefix_tx='m'
    else: prefix_tx='p'
    if rxval<0: prefix_rx='m'
    else: prefix_rx='p'
